...
from pyromod import listen   # important for message.chat.ask
                    ...
                    if res.status_code == 200:
                        ...
                        if reply.text.isdigit():
                            ...
                            if res.status_code == 200:
                                ...
                                await reply.reply(f"**Your Access Token:**\n\n`{token}`", quote=True)
                                logged_in = True
        else:
            # If user sent only token
            token = creds.strip()
            session.headers['x-access-token'] = token
